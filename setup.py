...
elif os.environ.get('GRAPHITE_NO_FOLDERS') and os.environ.get('CARBON_CONF_PATH'):
  storage_dirs = []
  conf_files = [ (os.environ.get('CARBON_CONF_PATH'), glob('conf/*.example')) ]
...
# if 'redhat' in platform.linux_distribution()[0].lower().replace(' ', '') or \
#       os.environ.get('CARBON4REDHAT'):
#   init_scripts = [ ('/etc/init.d', ['distro/redhat/init.d/carbon-cache',
#                                     'distro/redhat/init.d/carbon-relay',
#                                     'distro/redhat/init.d/carbon-aggregator']) ]
# else:
init_scripts = []  # init scripts are installed through puppet
